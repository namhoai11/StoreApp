...
import com.example.storeapp.data.local.AdminBottomNavigationItemsProvider
...
fun AdminBottomNavigationBar(
    ...
    NavigationBar(modifier = modifier) {
        AdminBottomNavigationItemsProvider.navigationItemList.forEach { navItem ->
            NavigationBarItem(
...
fun PreviewAdminBottomNavigationBar() {
    AdminBottomNavigationBar(
        navController = rememberNavController(), // Không ảnh hưởng trong Preview
        currentRoute = ManegeAdminDestination.route, // Thiết lập một route cố định để kiểm tra Preview
        modifier = Modifier
    )
}